...
- [ ] scrypt KEK:passphrase 經 scrypt(n=2**15,r=8,p=1) 派生 KEK,以 AES-GCM 包/解 DEK;定義 KeyEncapsulation 抽象介面供 P1 換 ML-KEM
- [ ] 檔案層:encrypt file→file.qvt、decrypt 還原原檔;大檔以固定塊 streaming,不整檔載入記憶體
- [ ] CLI:qvault encrypt/decrypt/inspect(argparse);inspect 只印 header 與演算法,錯密碼給乾淨錯誤不吐 traceback
